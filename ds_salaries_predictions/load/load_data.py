...
class DataRetriever:
    ...
    def _get_employment_type_desc(self, employee):
        logger.info("Transform the employment type to description")
        """
    ...
    def _get_level(self, employee):
        logger.info("Transform the level to description")
        """
    ...
    def retrieve_data(self):
        ...
        logger.info("Data loaded from " + self.url)
        ...
        if not os.path.exists(self.DATASETS_DIR):
            ...
            logger.info("Directory "+self.DATASETS_DIR+ "created successfully.")
            print(f"Directory '{self.DATASETS_DIR}' created successfully.")
        else:
            logger.info("Directory "+self.DATASETS_DIR+ "already exists.")
            print(f"Directory '{self.DATASETS_DIR}' already exists.")

        # Save data to CSV file
        data.to_csv(self.DATASETS_DIR + self.RETRIEVED_DATA, index=False)

        logger.info("Data stored in " + self.DATASETS_DIR + self.RETRIEVED_DATA)
